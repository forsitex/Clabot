# 📚 Clabot - Documentație Completă

**Betfair Bot cu Strategie de Progresie**
Versiune: 2.0 | Data: 30 Noiembrie 2025
...
**Clabot** este un bot automat de pariuri sportive care:

- Se conectează la **Betfair API**
- Gestionează echipe și meciuri în **Google Sheets**
...
┌─────────────────────────────────────────────────────────────┐
│                        DASHBOARD (Vue.js)                    │
│  - Teams Management  - Settings  - History  - Logs          │
└────────────────────┬────────────────────────────────────────┘
                     │ HTTP/WebSocket
┌────────────────────▼────────────────────────────────────────┐
│                    BACKEND (FastAPI)                         │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐      │
│  │  Bot Engine  │  │   Staking    │  │   Settings   │      │
│  │              │  │   Service    │  │   Manager    │      │
│  └──────┬───────┘  └──────────────┘  └──────────────┘      │
│         │                                                     │
│  ┌──────▼───────┐  ┌──────────────┐  ┌──────────────┐      │
│  │   Betfair    │  │Google Sheets │  │  Scheduler   │      │
│  │    Client    │  │    Client    │  │(APScheduler) │      │
│  └──────────────┘  └──────────────┘  └──────────────┘      │
└─────────┬───────────────┬────────────────────────────────────┘
          │               │
┌─────────▼─────┐  ┌──────▼──────┐
│  Betfair API  │  │Google Sheets│
│  (Romania)    │  │   (Clabot)  │
└───────────────┘  └─────────────┘
```
...
**Miză Inițială: 5 RON | Cotă medie: 1.5**

| Step | Pierdere Cumulată | Cotă | Miză Calculată | Rezultat | Profit/Pierdere | Nou Cumulative |
| ---- | ----------------- | ---- | -------------- | -------- | --------------- | -------------- |
| 0    | 0                 | 1.5  | 5              | LOST     | -5              | 5              |
| 1    | 5                 | 1.5  | 15             | LOST     | -15             | 20             |
| 2    | 20                | 1.5  | 45             | LOST     | -45             | 65             |
| 3    | 65                | 1.5  | 135            | WIN      | +67.5           | 0 (RESET)      |

**Total investit:** 5 + 15 + 45 + 135 = 200 RON
**Câștig:** 135 × 1.5 = 202.5 RON
**Profit net:** 202.5 - 200 = **2.5 RON** ✅
...
- ✅ Actualizare automată `last_stake` după plasare pariu
- ✅ `last_stake` = 0 pentru echipe noi
...
- ✅ Fix `get_pending_bets()` să accepte `team_name`
